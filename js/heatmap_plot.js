...
$.get("plot_layout_adjuster.html", function (plot_layout_adjuster_string) {
    ...
    $.ajax({
        url: "js/plot_layout_adjuster1.js", converters: { 'text script': function (text) { return text; } }, success: function (plot_layout_adjuster1) {
            ...
            console.log("adjusted_heatmap_plot_layout_adjuster1_done")

            // assign the default value for heatmap plot
            ocpu.call("call_fun", {
                parameter: {
                    user_id: localStorage.user_id,
                    fun_name: "get_heatmap_plot_style"
                }
            }, function (session) {
                session.getObject(function (heatmap_plot_obj) {
                    heatmap_plot_obj = prepare_layout(heatmap_plot_obj)
                    console.log(2)
                    ...
                    console.log("adjusted_heatmap_plot_layout_adjuster2_before")
                    $.ajax({
                        url: "js/plot_layout_adjuster2.js", converters: { 'text script': function (text) { return text; } }, success: function (plot_layout_adjuster2) {
                            ...
                            console.log("adjusted_heatmap_plot_layout_adjuster2_done")
                            // initialize the traces on HTML.
                            $("#show_sample_label").prop("checked", heatmap_plot_obj.traces.show_sample_label).change();
                            $("#show_compound_label").prop("checked", heatmap_plot_obj.traces.show_compound_label).change();
            ...
            console.log("adjusted_heatmap_plot_layout_adjuster3")
                        ...
                        console.log("adjusted_heatmap_plot_layout_adjuster3_done")
                        ...
                        if ($("#heatmap_plot_explanation_html").length == 0) {
                            $("#heatmap_plot_explanation").append("<div id='heatmap_plot_explanation_html'></div>")
                        }
                        $("#heatmap_plot_explanation_html").html(obj_heatmap_plot.results_description[1])
