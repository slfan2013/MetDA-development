        ...
        url: "js/plot_layout_adjuster1.js", converters: { 'text script': function (text) { return text; } }, success: function (plot_layout_adjuster1) {
            ...
            ocpu.call("call_fun", {
                ...
                session.getObject(function (volcano_plot_obj) {
                    volcano_plot_obj = prepare_layout(volcano_plot_obj)
                    ...
                    $.ajax({
                        url: "js/plot_layout_adjuster2.js", converters: { 'text script': function (text) { return text; } }, success: function (plot_layout_adjuster2) {
                            ...
                            $("#sig_pos").spectrum({
                                color: volcano_plot_obj.traces.colors[0],
                                showPalette: true,
                                palette: color_pallete
                            })
                            $("#sig_pos_size").val(volcano_plot_obj.traces.sizes[0])
                            $("#sig_pos_shape").val(volcano_plot_obj.traces.shapes[0])
                            $("#sig_pos_shape").selectpicker('refresh')

                            $("#sig_neg").spectrum({
                                color: volcano_plot_obj.traces.colors[1],
                                showPalette: true,
                                palette: color_pallete
                            })
                            $("#sig_neg_size").val(volcano_plot_obj.traces.sizes[1])
                            $("#sig_neg_shape").val(volcano_plot_obj.traces.shapes[1])
                            $("#sig_neg_shape").selectpicker('refresh')

                            $("#sig_not_pos_neg").spectrum({
                                color: volcano_plot_obj.traces.colors[2],
                                showPalette: true,
                                palette: color_pallete
                            })
                            $("#sig_not_pos_neg_size").val(volcano_plot_obj.traces.sizes[2])
                            $("#sig_not_pos_neg_shape").val(volcano_plot_obj.traces.shapes[2])
                            $("#sig_not_pos_neg_shape").selectpicker('refresh')

                            $("#not_sig_pos").spectrum({
                                color: volcano_plot_obj.traces.colors[3],
                                showPalette: true,
                                palette: color_pallete
                            })
                            $("#not_sig_pos_size").val(volcano_plot_obj.traces.sizes[3])
                            $("#not_sig_pos_shape").val(volcano_plot_obj.traces.shapes[3])
                            ...
                            $("#not_sig_neg").spectrum({
                                color: volcano_plot_obj.traces.colors[4],
                                showPalette: true,
                                palette: color_pallete
                            })
                            $("#not_sig_neg_size").val(volcano_plot_obj.traces.sizes[4])
                            $("#not_sig_neg_shape").val(volcano_plot_obj.traces.shapes[4])
                            $("#not_sig_neg_shape").selectpicker('refresh')

                            $("#not_sig_not_pos_neg").spectrum({
                                color: volcano_plot_obj.traces.colors[5],
                                showPalette: true,
                                palette: color_pallete
                            })
                            $("#not_sig_not_pos_neg_size").val(volcano_plot_obj.traces.sizes[5])
                            $("#not_sig_not_pos_neg_shape").val(volcano_plot_obj.traces.shapes[5])
                            $("#not_sig_not_pos_neg_shape").selectpicker('refresh')



                            $("#significancy_line_color").spectrum({
                                color: volcano_plot_obj.traces.significancy_line_color,
                                showPalette: true,
                                palette: color_pallete
                            })
                            $("#significancy_line_width").val(volcano_plot_obj.traces.significancy_line_width)
                            $("#significancy_line_dash").val(volcano_plot_obj.traces.significancy_line_dash)
            ...
            gather_page_information_to_volcano_plot = function () {
                ...
                $.ajax({
                    url: "js/plot_layout_adjuster3.js", converters: { 'text script': function (text) { return text; } }, success: function (plot_layout_adjuster3) {
                        ...
                        if ($("#volcano_plot_explanation_html").length == 0) {
                            $("#volcano_plot_explanation").append("<div id='volcano_plot_explanation_html'></div>")
                        }
                        $("#volcano_plot_explanation_html").html(obj_volcano_plot.results_description[1])
