...
initiateUrl <- URLencode("http://tempusername:temppassword@localhost:5985/metda_userinfo/initiate")
initiateList <- jsonlite::fromJSON(initiateUrl, simplifyVector = FALSE)
table_name = paste0("metda_userinfo_",new_user_id,".csv")

data.table::fwrite(data.table::fread(URLencode("http://tempusername:temppassword@localhost:5985/metda_userinfo/initiate/metda_userinfo_initiate.csv")),table_name)


projectUrl <- URLencode(paste0("http://tempusername:temppassword@localhost:5985/metda_userinfo"))
