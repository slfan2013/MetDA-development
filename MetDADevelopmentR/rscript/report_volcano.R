# report_volcano <- function(project_id = "report volcano1565300102", fold_id = "Volcano Plot1565300147", table_index = 1, figure_index = 1, doc = NULL) {
  ...
  doc <- doc %>%
    body_add_par("Volcano Plot Visualization Summary ", style = "heading 1") %>%
  ...
  if(is.null(parameters$p_value_data_treatment)){ # this means this is in and out. User uploaded the dataset. So we don't know p-values and fold change comparing what.
    doc <- doc %>%
      ...
      slip_in_text(parameters$volcano_plot$p_value_cut_off, style = "Default Paragraph Font", pos = "after") %>%
      slip_in_text(".", style = "Default Paragraph Font", pos = "after")


    doc <- doc %>%
      body_add_par("Cut-off: ", style = "Normal") %>%
      slip_in_text(paste0(signif(as.numeric(parameters$volcano_plot$fold_change_cut_off), 2), " and ", signif(1 / as.numeric(parameters$volcano_plot$fold_change_cut_off),2)), style = "Default Paragraph Font", pos = "after") %>%
      slip_in_text(".", style = "Default Paragraph Font", pos = "after")
  ...
  categories <- sapply(parameters$volcano_plot$data, function(data) unlist(data$x))
